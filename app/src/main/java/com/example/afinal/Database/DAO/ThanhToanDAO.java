...
public class ThanhToanDAO {
    ...
    public ThanhToan getByMaThueXe(int maThueXe) {
        open();
        ThanhToan thanhToan = null;
        try (Cursor c = db.rawQuery("SELECT * FROM THANHTOAN WHERE MaThueXe = ? ORDER BY NgayThucHien DESC LIMIT 1",
                new String[]{String.valueOf(maThueXe)})) {
            if (c.moveToFirst()) {
                thanhToan = getThanhToanFromCursor(c);
                Log.d(TAG, "Found payment for MaThueXe " + maThueXe + ": " +
                        "MaThanhToan=" + thanhToan.getMaThanhToan() +
                        ", NgayThucHien=" + thanhToan.getNgayThucHien() +
                        ", PhuongThuc=" + thanhToan.getPhuongThuc() +
                        ", TrangThai=" + thanhToan.getTrangThai());
            } else {
                Log.d(TAG, "No payment found for MaThueXe " + maThueXe);
            }
        } catch (SQLiteException e) {
    ...
    public List<ThanhToan> getAllByMaThueXe(int maThueXe) {
        open();
        List<ThanhToan> list = new ArrayList<>();
        try {
            Cursor cursor = db.query("ThanhToan", null, "MaThueXe = ?",
                    new String[]{String.valueOf(maThueXe)}, null, null, "NgayThucHien DESC");
            ...
            Log.e(TAG, "Error getting payments by MaThueXe: " + e.getMessage());
            e.printStackTrace();
        }
        close();
        return list;
